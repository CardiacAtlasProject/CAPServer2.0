...
public class PACSConfiguration {
	...
	private PACSCAPDatabaseSynchronizerRemote sync;
	...
	public PACSConfiguration() throws Exception {
		if (aetitle == null) {
			...
			if (sync != null) {
				aetitle = sync.getAetitle();
				hostname = sync.getHostname();
				port = sync.getPort();
				wadoPort = sync.getWadoPort();
				dcmprotocol = sync.getPacsProtocol();
			}
			Logger log = Logger.getLogger(DCMAccessManager.class.getSimpleName());
			if (sync == null)
				log.log(Level.INFO, "****************************************** SYNC is NULL");
			log.log(Level.INFO, "Configuring PACS with " + aetitle + "\t" + hostname + "\t" + port + "\t" + wadoPort + "\t" + dcmprotocol);
		}
	}

	/**
	 * Getter for ServerConfiguration object.It reads the element values from
	 * the xml document.Creates a new ServerConfiguration object, sets the
	 * properties of serverConfiguration and returns the serverConfiguration.
	 * 
