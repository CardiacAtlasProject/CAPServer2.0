...
public class DCMAccessManager {
	 ...
	 * the url filed.
	 */
	static final String CALLING_AET = "CAPQR";
	/**
		...
		try {
			...
			if ((value = cfg.getProperty("tls-key")) != null) {
				try {
					...
					log.log(Level.FINE, "Wrong value for tls-key: " + value + ". tls-key was set to default value.");
				}
			...
			if ((value = cfg.getProperty("tls-cacerts")) != null) {
				try {
					...
					log.log(Level.FINE, "Wrong value for tls-cacerts: " + value + ". tls-cacerts was set to default value.");
				}
	...
	public void store(Dataset ds) throws InterruptedException, IOException, ConnectException, ParseException {
		PresContext pc = null;
		log.log(Level.FINE, "In store call");
		...
		log.log(Level.FINE, "SOP Class UID " + sopClassUID);
		log.log(Level.FINE, "SOP Instance UID " + sopInstUID);
		...
		log.log(Level.FINE, CALLING_AET + " is storing a dataset");
		...
		case 0:
			break;
		default:
			log.log(Level.FINE, "STORE failed: " + Integer.toHexString(status));
		}
	}
	...
	public Vector<Dataset> retrieve(Dataset ds) throws ConnectException, IOException, InterruptedException {
		...
		// Get the Study Instance UID of the study to move
		...
		for (int i = 0; i < dimseList.size(); ++i) {
			...
			if (((Dimse) dimseList.get(i)).getDataset() == null) {
				log.log(Level.FINEST, "              Dataset created successfully          ");
			}
		...
		case 0:
			break;
		case 45056:
			log.log(Level.FINE, "One or more failures during move of " + prompt);
			break;
		default:
			log.log(Level.SEVERE, "Failed to move " + prompt + "\n\t error tstatus: " + Integer.toHexString(status));
		}

		log.log(Level.FINE, "The move size is : " + datasetVector.size());
		return datasetVector;
	}
	...
	public Vector<Dataset> move(Dataset ds) throws ConnectException, InterruptedException, IOException {
		...
		Vector<Dataset> datasetVector = new Vector<Dataset>();
		datasetVector.add(dms.getDataset());
		// PS 3.7 - 9.3.4 C-MOVE PROTOCOL, 9.3.4.2 C-MOVE-RSP
		int status = rspCmd.getStatus();
		switch (status) {
		case 0:
	...
	public static Vector<PatientRecord> getPatients(int lookBack) throws Exception {
		...
		for (int dataSetCount = 0; dataSetCount < numberofStudies; ++dataSetCount) {
			...
			try {
				...
				log.log(Level.FINE, "Error occured for patient ID " + patientID + " record number " + dataSetCount);
				exx.printStackTrace();
			}
	...
	public static Vector<StudyRecord> getPatientStudies(String patientID, long lookBack) throws Exception {
		...
		for (int dataSetCount = 0; dataSetCount < numberofStudies; ++dataSetCount) {
			...
			try {
				...
				Logger log = Logger.getLogger(DCMAccessManager.class.getSimpleName());
				log.log(Level.FINE, "Error occured for Study SOP ID " + sid + " record number " + dataSetCount);
				exx.printStackTrace();
			}
	...
	public static Vector<InstanceRecord> getStudySeries(String studyID, String seriesUID, long lookBack) throws Exception {

		Hashtable<String, InstanceRecord> instances = new Hashtable<String, InstanceRecord>();

		DCMConfigProperties properties = new DCMConfigProperties(StorageService.class.getResource("/resources/Series.cfg"));

		properties.put("key.StudyID", studyID);
		properties.put("key.SeriesInstanceUID", seriesUID);
		if (lookBack > 0) {
			properties.put("key.StudyDate", ""+lookBack);
		}
	...
	public static Vector<InstanceRecord> getStudyInstances(String studyInstanceUID, long lookBack) throws Exception {

		Hashtable<String, InstanceRecord> instances = new Hashtable<String, InstanceRecord>();

		DCMConfigProperties properties = new DCMConfigProperties(StorageService.class.getResource("/resources/Image.cfg"));
		if(studyInstanceUID!=null)
			properties.put("key.StudyInstanceUID", studyInstanceUID);
