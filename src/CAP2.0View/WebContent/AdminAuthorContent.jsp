...
<script>
	...
	function processMetaDataRequest(button) {
		...
		if (text == "Upload") {
			selectedStudy = value;
			$.mobile.changePage(
					"./uploadStudyMetaData.jsp?action=addStudyMetaData", {
						transition : 'pop',
						role : 'dialog'
					});
			return;
		...
		if (text == "Replace" || text == "Delete" || text == "Download") {
			...
			if (text == "Replace") {
				selectedStudy = value;
				$.mobile.changePage(
						"./uploadStudyMetaData.jsp?action=replaceStudyMetaData&descriptor="
								+ selection, {
							transition : 'pop',
							role : 'dialog'
						});
			...
			selectedModel = value;
			$.mobile.changePage(
					"./uploadModelMetaData.jsp?action=AddToMetadata", {
						transition : 'pop',
						role : 'dialog'
					});
			...
			selectedModel = value;
			$.mobile.changePage(
					"./uploadModelMetaData.jsp?action=ReplaceMetadata", {
						transition : 'pop',
						role : 'dialog'
					});
			...
			selectedModel = value;
			$.mobile.changePage(
					"./uploadModelMetaData.jsp?action=UploadMetadata", {
						transition : 'pop',
						role : 'dialog'
					});
	...
	function modelformat(d) {
		...
		if (d.HASMETADATA == "0") {//When model has not metadata the value is 1
			mytable += '<td>';
			mytable += '<button class="ui-btn" onclick="processMetaDataRequest(this)" value="'
					+ d.MODELID + '" >Add to Meta Data</button>';
			mytable += '<button class="ui-btn" onclick="processMetaDataRequest(this)" value="'
					+ d.MODELID + '" >Replace Meta Data</button>';
			mytable += '<button class="ui-btn" onclick="processMetaDataRequest(this)" value="'
					+ d.MODELID + '" >Delete Meta Data</button>';
			mytable += '<button class="ui-btn" onclick="processMetaDataRequest(this)" value="'
					+ d.MODELID + '" >Update Comments</button>';
			mytable += '<button class="ui-btn" onclick="processMetaDataRequest(this)" value="'
					+ d.MODELID + '" >Download Meta Data</button>';
			mytable += '</td></tr>';
		} else {
			mytable += '<td><button class="ui-btn" onclick="processMetaDataRequest(this)" value="'
					+ d.MODELID + '" >Upload Meta Data</button>';
			mytable += '<button class="ui-btn" onclick="processMetaDataRequest(this)" value="'
					+ d.MODELID + '" >Update Comments</button></td></tr>';
		}

		if (d.HASMETADATA == "0") {
			mytable += '<td><button class="ui-btn" onclick="processMetaDataRequest(this)" value="'
					+ d.MODELID + '" >Download Meta Data</button></td></tr>';
		} else {
			mytable += '</tr>'
		}
	...
	function format(d) {
		...
		if (metadataj.length == 0) {
			...
			options += '<td><button class="ui-btn" onclick="processMetaDataRequest(this)" value="'
					+ d.UID + '" >Upload</button></td>';
			...
			options += '</select></td>';
			options += '<td><button class="ui-btn" onclick="processMetaDataRequest(this)" value="'
					+ d.UID + '" >Replace</button></td>';
			options += '<td><button class="ui-btn" onclick="processMetaDataRequest(this)" value="'
					+ d.UID + '" >Delete</button></td>';
			options += '<td><button class="ui-btn" onclick="processMetaDataRequest(this)" value="'
					+ d.UID + '" >Upload</button></td>';
			options += '<td><button class="ui-btn" onclick="processMetaDataRequest(this)" value="'
					+ d.UID + '" >Download</button></td>';
			metadata = options + '</tr></table>';
		}

		var mytable = '<table><tr><td>' + metadata + '</td></tr>';
		...
		JS.text = '$("#STUDIES' + d.ID
				+ '").DataTable(); $(".metadataselect").selectmenu();';
	...
	$(".uploadModels").bind("click", function(event, ui) {
		$.mobile.changePage("./uploadModel.jsp", {
			transition : 'pop',
			role : 'dialog'
		});
	});
	...
	$('.deleteModels')
			.bind(
					...
					function(event, ui) {
						...
						var requestObj = new Object();
						requestObj["removeModel"] = JSON
								.stringify(activeModels);
