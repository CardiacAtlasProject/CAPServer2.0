...
<script>
	...
	function processMetaDataRequest(button){
		...
		if(text=="Upload"){
			 selectedStudy = value;
			 $.mobile.changePage("./uploadStudyMetaData.jsp?action=addStudyMetaData", {transition: 'pop', role: 'dialog'});
			 return;
		}
		//These options are available if any metadata is available
	    if(text=="Replace"||text=="Delete"||text=="Download"){
		 var e = document.getElementById("select-choice-"+value);
		 var selection = e.options[e.selectedIndex].text;
		 if(text=="Replace"){
			 selectedStudy = value;
			 $.mobile.changePage("./uploadStudyMetaData.jsp?action=replaceStudyMetaData&descriptor="+selection, {transition: 'pop', role: 'dialog'});
		 }else if(text=="Delete"){
			selectedStudy = value;
			deleteStudyMetaData(value,selection);
	    ...
	    }else if(text=="Add to Meta Data"){
		    selectedModel = value;
		    $.mobile.changePage("./uploadModelMetaData.jsp?action=AddToMetadata", {transition: 'pop', role: 'dialog'});
		}else if(text=="Replace Meta Data"){
		    selectedModel = value;
		    $.mobile.changePage("./uploadModelMetaData.jsp?action=ReplaceMetadata", {transition: 'pop', role: 'dialog'});
		}else if(text=="Upload Meta Data"){
		    selectedModel = value;
		    $.mobile.changePage("./uploadModelMetaData.jsp?action=UploadMetadata", {transition: 'pop', role: 'dialog'});
		}else if(text=="Delete Meta Data"){
		    selectedModel = value;
		    deleteModelMetaData();
		}else if(text=="Download Meta Data"){
		    selectedModel = value;
		    downloadModelMetaData();
		}else if(text=="Update Comments"){
		    selectedModel = value;
		    updateModelComments();
		}else{
			console.debug(text)
	...
	function deleteStudyMetaData(studyid,selection){
        var requestObj = new Object();
		requestObj["deleteStudyMetaData"]=studyid;
		...
		$.ajax({
		    ...
		    {
			    //console.debug(data);
				alert("Meta data deleted");
				$('#dicomtable').DataTable().ajax.reload(null, false ); // user paging is not reset on reload
		    },
    ...
    function deleteModelMetaData(){
        var requestObj = new Object();
		requestObj["deleteModelMetaData"]=selectedModel;
		$.ajax({
		    url : "./CAPModel",
		    type: "POST",
		    data : requestObj,
		    ...
		    success: function(data, textStatus, jqXHR)
		    {
			    //console.debug(data);
				alert("Meta data deleted");
				$('#modeltable').DataTable().ajax.reload(null, false ); // user paging is not reset on reload
		    },
		    error: function (jqXHR, textStatus, errorThrown)
		    {
			    alert('Internal server error!Please contact the Administrator');
    ...
    function updateModelComments(){
		var comments = $('textarea#modelcommentsfor'+selectedModel).val();
        var requestObj = new Object();
		requestObj["updateModelComments"]=selectedModel;
		requestObj["comment"]=encodeURIComponent(comments);
		$.ajax({
		    url : "./CAPModel",
		    type: "POST",
		    data : requestObj,
	...
	function modelformat(d) {
		...
		var mytable = '<table><tr><td>Comments: <textarea rows="10" cols="50" id="modelcommentsfor'+d.MODELID+'">'
		 ...
		 if(d.HASMETADATA=="0"){//When model has not metadata the value is 1
			mytable += '<td>';
			mytable += '<button class="ui-btn" onclick="processMetaDataRequest(this)" value="'+d.MODELID+'" >Add to Meta Data</button>';
			mytable += '<button class="ui-btn" onclick="processMetaDataRequest(this)" value="'+d.MODELID+'" >Replace Meta Data</button>';
			mytable += '<button class="ui-btn" onclick="processMetaDataRequest(this)" value="'+d.MODELID+'" >Delete Meta Data</button>';
			mytable += '<button class="ui-btn" onclick="processMetaDataRequest(this)" value="'+d.MODELID+'" >Update Comments</button>';
			mytable += '<button class="ui-btn" onclick="processMetaDataRequest(this)" value="'+d.MODELID+'" >Download Meta Data</button>';
			mytable += '</td></tr>';
		 }else{
				mytable += '<td><button class="ui-btn" onclick="processMetaDataRequest(this)" value="'+d.MODELID+'" >Upload Meta Data</button>';
				mytable += '<button class="ui-btn" onclick="processMetaDataRequest(this)" value="'+d.MODELID+'" >Update Comments</button></td></tr>';
		 }
	...
	function format(d) {
		...
		if (metadataj.length == 0){
			...
			options += '<td><button class="ui-btn" onclick="processMetaDataRequest(this)" value="'+d.UID+'" >Upload</button></td>';
			...
			options += '</select></td>';
			options += '<td><button class="ui-btn" onclick="processMetaDataRequest(this)" value="'+d.UID+'" >Replace</button></td>';
			options += '<td><button class="ui-btn" onclick="processMetaDataRequest(this)" value="'+d.UID+'" >Delete</button></td>';
			options += '<td><button class="ui-btn" onclick="processMetaDataRequest(this)" value="'+d.UID+'" >Upload</button></td>';
	...
	$(".uploadModels").bind("click", function(event, ui) {
		$.mobile.changePage("./uploadModel.jsp", {transition: 'pop', role: 'dialog'});
	});
	...
	$(".unSelectallModels").bind("click", function(event, ui) {
		...
		$('.deleteModels').bind('click',function(event, ui){
			if(activeModels.length==0){
				alert("Atleast one model should be chosen!!");
				return;
			}

	        var requestObj = new Object();
			requestObj["removeModel"]=JSON.stringify(activeModels);
			//console.debug(requestObj);
			$.ajax({
			    url : "./CAPModel",
			    type: "POST",
			    data : requestObj,
			    ...
			    {
				    //console.debug(data);
					alert("Success fully removed model(s)");
					$('#modeltable').DataTable().ajax.reload(null, false ); // user paging is not reset on reload
			    },
			    error: function (jqXHR, textStatus, errorThrown)
			    {
			...
			<div>
				<fieldset data-role="controlgroup" data-type="horizontal" class="localnav" style="float: right" id="dicomcontrols">
					<input type="button" value="Un Select All" class="unSelectallModels" data-mini="true"> <input type="button" value="Select All"
						class="selectallModels" data-mini="true"> <input type="button" value="Download" class="downloadModels" data-mini="true"> <input
						type="button" value="Upload" class="uploadModels" data-mini="true"> <input type="button" value="Delete" class="deleteModels" data-mini="true">
				</fieldset>
			</div>
