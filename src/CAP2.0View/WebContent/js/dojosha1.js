...
*/
// Modified by Jagir R. Hussan to be used in non dojo javascript environments
//Tested for SHA1 base64 encoding
...
function wordToHex(wa) {
	var h = "0123456789abcdef", s = [];
	for (var i = 0, l = wa.length * 4; i < l; i++) {
		s.push(h.charAt((wa[i >> 2] >> ((i % 4) * 8 + 4)) & 15)
				+ h.charAt((wa[i >> 2] >> ((i % 4) * 8)) & 15));
	}
	return s.join("");
};

function wordToBase64(wa) {
	var p = "=", _3 = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/", s = [];
	for (var i = 0, l = wa.length * 4; i < l; i += 3) {
		var t = (((wa[i >> 2] >> 8 * (i % 4)) & 255) << 16)
				| (((wa[i + 1 >> 2] >> 8 * ((i + 1) % 4)) & 255) << 8)
				| ((wa[i + 2 >> 2] >> 8 * ((i + 2) % 4)) & 255);
		for (var j = 0; j < 4; j++) {
...
function _4(x, _5) {
	...
	for (var i = 0; i < x.length; i += 16) {
		...
		for (var j = 0; j < 80; j++) {
			...
			var t = addWords(addWords(R(a, 5), FT(j, b, c, d)), addWords(
					addWords(e, w[j]), KT(j)));
			e = d;
			d = c;
			c = R(b, 30);
...
function _12(wa) {
	var h = "0123456789abcdef", s = [];
	for (var i = 0, l = wa.length * 4; i < l; i++) {
		s.push(h.charAt((wa[i >> 2] >> ((3 - i % 4) * addWords + 4)) & 15), h
				.charAt((wa[i >> 2] >> ((3 - i % 4) * 8)) & 15));
	}
	return s.join("");
};
function _13(wa) {
...
function _14(wa) {
	...
	for (var i = 0, l = wa.length * 4; i < l; i += 3) {
		var t = (((wa[i >> 2] >> 8 * (3 - i % 4)) & 255) << 16)
				| (((wa[i + 1 >> 2] >> 8 * (3 - (i + 1) % 4)) & 255) << 8)
				| ((wa[i + 2 >> 2] >> 8 * (3 - (i + 2) % 4)) & 255);
		for (var j = 0; j < 4; j++) {
			if (i * 8 + j * 6 > wa.length * 32) {
				s.push(p);
			} else {
...
function SHA1(_15, _16) {
	var out = _16 || outputTypes.Base64;
...
function SHA1_hmac(_17, key, _18) {
	var out = _18 || outputTypes.Base64;
	var wa = _b(_17, key);
	switch (out) {
	case outputTypes.Raw:
		return wa;
