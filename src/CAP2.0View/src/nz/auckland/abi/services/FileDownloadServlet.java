 ...
 */
@WebServlet(urlPatterns = "/resource/*", asyncSupported = true)
public class FileDownloadServlet extends HttpServlet {
	...
	private static final Logger log = Logger.getLogger("Download Manager");
	private static int DEFAULT_BUFFER_SIZE = 10240; // ..bytes = 10KB.
	private long DEFAULT_EXPIRE_TIME = 86400000L; // ..ms =  1 day.
	...
	@EJB
	private FileResourcesManagerRemote fileManager;
	...
	private void processRequest(HttpServletRequest request, HttpServletResponse response, boolean content) throws IOException {
		...
		//
		String resourceid = URLDecoder.decode(request.getPathInfo(), "UTF-8");
		File resource = null;
		if (resourceid != null) {
			while (resource == null) {//Don this way to handle concurrent access exceptions
				try {
					resource = fileManager.getResourceLocation(resourceid.substring(1)); // Remove
																							// the
																							// /
					String failedResourceMessage = fileManager.checkFailure(resourceid);
					if (failedResourceMessage != null) {
						response.sendError(HttpServletResponse.SC_INTERNAL_SERVER_ERROR, failedResourceMessage);
						return;
					}
					// Check if file is actually supplied to the request URL.
					if (resource == null) {
						// Do your thing if the file is not supplied to the
						// request URL.
						// Throw an exception, or send 404, or show
						// default/warning page, or
						// just ignore it.
						response.getWriter().println("<!DOCTYPE html><html><head><meta charset=\"UTF-8\"><title>Resource not found</title></head><body style=\"position: absolute; top: 50%; left: 50%; -moz-transform: translateX(-50%) translateY(-50%); -webkit-transform: translateX(-50%) translateY(-50%); transform: translateX(-50%) translateY(-50%);\">	<div >	<h1 >Resource, "+resourceid+", not found.</h1>	</div></body></html>");
						response.sendError(HttpServletResponse.SC_NOT_FOUND);
						return;
					}
				} catch (ConcurrentAccessException conc) {
		...
		// Wait for the resource, as sometimes the file is still under
		// construction
		int waitCtr = 30;//return error after 5 minute wait
		while (waitCtr>0 && !resource.exists()) {
			// Check if resource creation has failed
			String failedResourceMessage = fileManager.checkFailure(resourceid);
			if (failedResourceMessage != null) {
				response.getWriter().println("<!DOCTYPE html><html><head><meta charset=\"UTF-8\"><title>Resource creation failed</title></head><body style=\"position: absolute; top: 50%; left: 50%; -moz-transform: translateX(-50%) translateY(-50%); -webkit-transform: translateX(-50%) translateY(-50%); transform: translateX(-50%) translateY(-50%);\">	<div >	<h1 >Resource, "+resourceid+", not found. The request to generate the resource failed.</h1>	</div></body></html>");
				response.sendError(HttpServletResponse.SC_INTERNAL_SERVER_ERROR, failedResourceMessage);
				return;
			}
			try {
				log.log(Level.INFO, "File download waiting for resource " + resourceid+" alive for "+waitCtr);
		...
		if(resource.isDirectory()){ //Send a message asking the user to try again
			double prog = fileManager.getProgress(resourceid);
			boolean ret = true;
			if(prog>0.99){//If closer to completion give it another second
				try{
					Thread.sleep(1000);
				}catch(Exception ex1){
					
				}
				prog = fileManager.getProgress(resourceid);
				resource = fileManager.getResourceLocation(resourceid.substring(1));
			...
			if(ret){
				if(prog<0.0)
					prog = 0.0;
				prog = (int)(prog*100);
				response.getWriter().println("<!DOCTYPE html><html><head><meta charset=\"UTF-8\"><title>Download resource being generated!!</title></head><body	style=\"position: absolute; top: 50%; left: 50%; -moz-transform: translateX(-50%) translateY(-50%); -webkit-transform: translateX(-50%) translateY(-50%); transform: translateX(-50%) translateY(-50%);\">	<h4>		You are request, "+resourceid+" , is being processed.<br> Current progress is "+prog+"%. <br> The server may be busy or dataset is large. <br> Resources are alive for		atleast 24 hours post their creation. <br> Please try again later.	</h4></body></html>");
				return;
			}
		}
		...
		if(waitCtr==0){
			{
				fileManager.setResouceFailure(resourceid, "FILE NOT FOUND");
				response.sendError(HttpServletResponse.SC_NOT_FOUND, "Resource not found!!");
			}
		}

		DEFAULT_EXPIRE_TIME = fileManager.getResourceLife();
		DEFAULT_BUFFER_SIZE = (int)fileManager.getDownloadTokenSize();
		...
		// Encode filename specific to browser
		// http://stackoverflow.com/questions/5325322/java-servlet-download-filename-special-characters/13359949#13359949
		byte[] fileNameBytes = resource.getName().getBytes((isInternetExplorer) ? ("windows-1250") : ("utf-8"));
		...
		response.setHeader("Content-Disposition", disposition + ";filename=\"" + dispositionFileName + "\"");
		...
		try {
			// Open streams.
			input = new RandomAccessFile(resource, "r");
