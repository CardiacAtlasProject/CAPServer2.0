...
/**
 * Performance test for the BaselineDiagnosis entity.
 */
class BaselineDiagnosisGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the BaselineDiagnosis entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all baselineDiagnoses")
            .get("/api/baseline-diagnoses")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            ...
            .pause(10)
            .repeat(5) {
                exec(http("Get created baselineDiagnosis")
                .get("${new_baselineDiagnosis_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created baselineDiagnosis")
            .delete("${new_baselineDiagnosis_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
